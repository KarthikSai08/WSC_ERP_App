...
namespace WSC.Gateway.Infrastructure.Repositories
{
    public class RefreshTokenRepository : IRefreshTokenRepository
    {
        ...
        public async Task<RefreshToken?> GetRefreshTokenAsync(string token, CancellationToken ct)
        {
            ...
            var sql = "SELECT * FROM auth.RefreshTokens WHERE Token = @Token";

            return await con.QueryFirstOrDefaultAsync<RefreshToken>(sql, new { Token = token });
        }

        public async Task RevokeAllUserTokenAsync(string userId, CancellationToken ct)
        {
            using var con = _context.CreateConnection();
            var sql = "UPDATE auth.RefreshTokens SET IsRevoked = 1, RevokedAt = SYSUTCDATETIME() WHERE UserId = @UserId AND IsRevoked = 0";
        ...
        {
            using var con = _context.CreateConnection();
            var sql = "UPDATE auth.RefreshTokens SET IsRevoked = 1, RevokedAt = SYSUTCDATETIME(), ReplacedByToken = @ReplacedByToken WHERE Token = @Token ";
        ...
        {
            using var con = _context.CreateConnection();
            var sql = "INSERT INTO auth.RefreshTokens (UserId, Token, ExpiresAt) VALUES (@UserId, @Token, @ExpiresAt)";
